...
@test isapprox(Φ(θ, v), Φ(θ, v.a))
@test isapprox(G(θ, v), G(θ, v.a))
@test isapprox(G(θ, v), Φ(θ, v))
@test isapprox(hopfmap(v), hopfmap(v.a))
...
@test isapprox(Quaternion(-vec(v)[3], -vec(v)[4], vec(v)[1], vec(v)[2]), (G(ϵ, v) - G(0, v)) * (1 / ϵ), atol = _tolerance)
# Check if some tangent vector to the curve of points generated by the action, at the identity of the action, is in the vertical space
