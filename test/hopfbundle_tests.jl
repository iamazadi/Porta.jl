...
@test isapprox(zerovector, π✳(Dualquaternion(v, (Φ(ϵ, v) - Φ(0, v)) * (1 / ϵ))), atol = TOLERANCE)
...
@test isapprox(Quaternion(-vec(v)[3], -vec(v)[4], vec(v)[1], vec(v)[2]), (G(ϵ, v) - G(0, v)) * (1 / ϵ), atol = TOLERANCE)
# Check if some tangent vector to the curve of points generated by the action, at the identity of the action, is in the vertical space
@test isapprox(zerovector, π✳(Dualquaternion(v, (G(ϵ, v) - G(0, v)) * (1 / ϵ))), atol = TOLERANCE)
