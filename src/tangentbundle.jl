...
struct TangentBundle
    ...
    TangentBundle(lscene::LScene, name::String; segments::Int = 30, pathsegments::Int = 360, transparency::Bool = false,
        ...
        lines!(lscene, raya, color = segmentcolors, linewidth = linewidth, colorrange = (1, segments), colormap = colormap, transparency = transparency)
        lines!(lscene, rayb, color = segmentcolors, linewidth = linewidth, colorrange = (1, segments), colormap = colormap, transparency = transparency)
        lines!(lscene, rayc, color = segmentcolors, linewidth = linewidth, colorrange = (1, segments), colormap = colormap, transparency = transparency)
        lines!(lscene, rayd, color = segmentcolors, linewidth = linewidth, colorrange = (1, segments), colormap = colormap, transparency = transparency)
        lines!(lscene, fiber, color = GLMakie.@lift(collect(1:length($fiber))), linewidth = 2linewidth, colorrange = (1, segments), colormap = colormap, transparency = transparency)
        lines!(lscene, basepath, color = GLMakie.@lift(collect(1:length($basepath))), linewidth = linewidth / 2, colorrange = (1, pathsegments), colormap = colormap, transparency = transparency)
