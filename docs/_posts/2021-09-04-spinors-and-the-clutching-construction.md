...
Next, define the quotient map which formalizes the equivalence class in the vector bundle. It's the disjoint union of a direct product. Then, we see how using function f this trivial product turns into a non-trivial vector bundle.

$ \sigma: (U_{-} \times \mathbb{R}^2) \dot{\cup} (U_{+} \times \mathbb{R}^2) \to E_f $, where $ E_f = \tilde{E}/\sim $ and $ \bar{E} = (U_{-} \times \mathbb{R}^2) \dot{\cup} (U_{+} \times \mathbb{R}^2) $.

Natrually, the quotient map is going to decompose into two parts, because there are two domains: the northern hemisphere and the southern. $ [x, y, z, w]_{\mp} = \sigma_{\mp}(x, y, z, w) $. That's where we have two different quotient maps. Passing from one hemisphere to the other one, the extended clutching function determines how a vecttor in the tangent space needs to rotate. So we have $ [x, y, z, v]_{-} = [x, y, z, \bar{f}(x, y, z) \bullet v]_{+} $ where $ \bar{f} $ is the extended clutching function.

ext, using what we have define our formal bundle charts in both hemispheres, by taking vectors in the tangent space and taking them into a direct product between the domain on the sphere and the t-dimensional vector space. That requires using the inverse of the quotient map. $ \phi_{\pm}: E_f \to U_{\pm} \times \mathbb{R}^2 $ given by $ [x, y, z, v] \mapsto {\sigma_{\pm}}^{-1}([x, y, z, v]_{\pm}) $. These formal bundle charts are supposed to be smooth because when using the inverse of the quotiient map the clutching function comes into play acts as a natural and smooth way to reorient vectors.
$$ \phi_{+} \circ \phi_{-}(x, y, z, v) = {\sigma_{+}}^{-1} \circ \sigma_{-}(x, y, z, v)\\
= {\sigma_{+}}^{-1}[x, y, z, v]_{-}\\
= {\sigma_{+}}^{-1}[x, y, z, \bar{f}(x, y, z) \bullet v]_{+}\\
= (x, y, z, \bar{f}(x, y, z) \bullet v) $$

So that gives us a vector bundle $ E_f $ over the sphere with the base space: $ S^2 $, fiber space: $ \mathbb{R}^2 $, bundle charts: $ \phi_{+} and \phi_{-} $ and the projection map: $ \pi: E_f \to S^2 $ which removes the piece of information and uses the clutching function.

![vectorbundle](https://github.com/iamazadi/Porta.jl/raw/master/docs/_posts_images/vectorbundle.JPG)

### Summary
So, we saw how fermions are sections of vector bundles in the standard model of elementary particle physics. The, we saw how there are vector bundles that are trivial, but when they are glued together, we saw in a geometrical way how they can become spinorial. And that gives us a spinor field, as vectors on the base space. These are 1-component spinors because the twist requires only one variable to determine. Now, if we identify the the equator circle with $ S^1 $ and the twist with the Lie group $ SO(2) $ then we're going to have our function $ f: S^1 \to SO(2) \cong S^1 $.

![spinorfield](https://github.com/iamazadi/Porta.jl/raw/master/docs/_posts_images/spinorfield.JPG)
![flag](https://github.com/iamazadi/Porta.jl/raw/master/docs/_posts_images/flag.GIF)
