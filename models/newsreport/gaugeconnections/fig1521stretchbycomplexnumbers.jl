...
transform(θ::Float64, ϕ::Float64, α::Float64, interpolation::Float64) = interpolation .* twist(θ, ϕ, α)
...
calculateframe(frame::Int, interpolation::Float64) = begin
    ...
    x = real(p)
    y = imag(p)
    x, y = transform(x, y, 0.0, interpolation)
    _basepoint = ℝ³(x, y, 0.0)
    height = k * (stage - 1) + stageprogress * vec(f(x, y, k))[3]
    x = real(p)
    y = imag(p)
    x, y = transform(x, y, height / 2π, interpolation)
    _basepoint = ℝ³(x, y, 0.0)
    height = k * (stage - 1) + stageprogress * vec(f(x, y, k))[3]
    ...
    _upheadx = Point3f((f(transform(x + ϵ, y, height / 2π, interpolation)..., k) - f(transform(x, y, height / 2π, interpolation)..., k)) * (1 / ϵ))
    _upheady = Point3f((f(transform(x, y + ϵ, height / 2π, interpolation)..., k) - f(transform(x, y, height / 2π, interpolation)..., k)) * (1 / ϵ))
...
animate(frame::Int) = begin
    ...
    if stage < totalstages
        section1 = [ℝ³([transform(-θ, ϕ, 0.0, interpolation)...; 0.0]) for θ in lspace2, ϕ in lspace1]
        section2 = [f(transform(-θ, ϕ, k / distance, interpolation)..., k) + k * ẑ for θ in lspace2, ϕ in lspace1]
        section3 = [f(transform(-θ, ϕ, 2k / distance, interpolation)..., k) + 2k * ẑ for θ in lspace2, ϕ in lspace1]
        section4 = [f(transform(-θ, ϕ, 3k / distance, interpolation)..., k) + 3k * ẑ for θ in lspace2, ϕ in lspace1]
        section5 = [f(transform(-θ, ϕ, 4k / distance, interpolation)..., k) + 4k * ẑ for θ in lspace2, ϕ in lspace1]
        ...
        for index in 1:length(boundary_names)
            boundary = convert_to_geographic.(boundary_nodes[index])
            fiber = [ℝ³(transform(boundary[i][2], boundary[i][3], _distance / distance, interpolation)..., _distance) for i in eachindex(boundary), _distance in lspace]
            updatesurface!(fiber, fibersobservables[index])
        end

        basetail[], baseheadx[], baseheady[], uptail[], upheadx[], upheady[], height = calculateframe(frame, interpolation)
        spawn = true
        if spawn && frame % 5 == 0
            global interpolation = sin(progress * π)
        ...
        for index in eachindex(path[])
            _basetail, _baseheadx, _baseheady, _uptail, _upheadx, _upheady, _height = calculateframe(index, interpolation)
