...
animate(frame::Int) = begin
    ...
    section2 = [rotate(ℝ³([-θ; ϕ; height]), ℍ(height, ẑ)) for θ in lspace2, ϕ in lspace1]
    ...
    for index in 1:length(boundary_names)
        ...
        fiber2 = [rotate(ℝ³(vec(boundary[i])[2], vec(boundary[i])[3], _height), ℍ(_height, ẑ)) for i in eachindex(boundary), _height in lspace]
