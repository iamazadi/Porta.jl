...
for (i, scale1) in enumerate(collection)
    ...
    οline = lines!(lscene, οlinepoints[i], color = οlinecolors[i], linewidth = linewidth, colorrange = (1, 2segments), colormap = :spring)
    ιline = lines!(lscene, ιlinepoints[i], color = ιlinecolors[i], linewidth = linewidth, colorrange = (1, 2segments), colormap = :summer)
...
animate1(frame::Int) = begin
    ...
    κflagplanematrix = makeflagplane(𝕍(vec(κv)), 𝕍(LinearAlgebra.normalize(vec(_κ′ - _κ))), T, segments = segments)
    ωflagplanematrix = makeflagplane(𝕍(vec(ωv)), 𝕍(LinearAlgebra.normalize(vec(_ω′ - _ω))), T, segments = segments)
    τflagplanematrix = makeflagplane(𝕍(vec(τv)), 𝕍(LinearAlgebra.normalize(vec(_τ′ - _τ))), T, segments = segments)
...
animate(frame::Int) = begin
    ...
    for (i, scale1) in enumerate(collection)
        ...
        _τlinecolors = Int[]
        for (j, scale2) in enumerate(collection)
            οvector = M * normalize(ℍ(vec(scale1 * κv + scale2 * x)))
            ιvector = M * normalize(ℍ(vec(scale1 * κv + scale2 * -x)))
