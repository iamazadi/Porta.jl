...
eyeposition = LinearAlgebra.normalize([0.9, 0.3, 0.75]) .* 0.6
lookat = [-0.1, 0.1, 0.1]
up = [0.0; 0.0; 1.0]
...
O_B_R2 = [ê[1] ê[2] ê[3]]
B_O_R2 = LinearAlgebra.inv(O_B_R)
...
backgroundcolor2 = GLMakie.RGBf(0.0, 0.0, 0.0)
lscene = GLMakie.LScene(fig[1, 2], show_axis = true, scenekw = (lights = [pl, al], clear = true, backgroundcolor = backgroundcolor))
lscene2 = GLMakie.LScene(fig[1, 1], show_axis = true, scenekw = (lights = [pl, al], clear = true, backgroundcolor = backgroundcolor2))
...
pivot_observable2 = GLMakie.Observable(pivot)
point1_observable2 = GLMakie.Observable(p1)
point2_observable2 = GLMakie.Observable(p2)
point3_observable2 = GLMakie.Observable(p3)
point4_observable2 = GLMakie.Observable(p4)
...
robot2 = make_sprite(lscene2.scene, lscene2.scene, chassis_origin, chassis_rotation, chassis_scale, chassis_stl, chassis_colormap)
rollingwheel2 = make_sprite(lscene2.scene, robot2, rollingwheel_origin, rollingwheel_rotation, rollingwheel_scale, rollingwheel_stl, rollingwheel_colormap)
reactionwheel2 = make_sprite(lscene2.scene, robot2, reactionwheel_origin, reactionwheel_rotation, reactionwheel_scale, reactionwheel_stl, reactionwheel_colormap)
...
_ps = GLMakie.Observable([GLMakie.Point3f(R1_tail...), GLMakie.Point3f(R2_tail...), GLMakie.Point3f(R3_tail...), GLMakie.Point3f(R4_tail...)])
_ns = GLMakie.Observable([GLMakie.Vec3f(R1...), GLMakie.Vec3f(R2...), GLMakie.Vec3f(R3...),  GLMakie.Vec3f(R4...)])
...
pivotball2 = GLMakie.meshscatter!(lscene2, pivot_observable2, markersize = 0.01, color = :gold)
ball12 = GLMakie.meshscatter!(lscene2, point1_observable2, markersize = 0.01, color = :orange)
ball22 = GLMakie.meshscatter!(lscene2, point2_observable2, markersize = 0.01, color = :lime)
ball32 = GLMakie.meshscatter!(lscene2, point3_observable2, markersize = 0.01, color = :pink)
ball42 = GLMakie.meshscatter!(lscene2, point4_observable2, markersize = 0.01, color = :purple)
...
pivot_ps2 = GLMakie.Observable([pivot_observable2[], pivot_observable2[], pivot_observable2[]])
pivot_ns2 = GLMakie.Observable(map(x -> x .* smallarrowscale, ê))
ps12 = GLMakie.Observable([point1_observable2[], point1_observable2[], point1_observable2[]])
ps22 = GLMakie.Observable([point2_observable2[], point2_observable2[], point2_observable2[]])
ps32 = GLMakie.Observable([point3_observable2[], point3_observable2[], point3_observable2[]])
ps42 = GLMakie.Observable([point4_observable2[], point4_observable2[], point4_observable2[]])
ns12 = GLMakie.Observable(map(x -> smallarrowscale .* B_O_R * x, [B_A1_R * ê[1], B_A1_R * ê[2], B_A1_R * ê[3]]))
ns22 = GLMakie.Observable(map(x -> smallarrowscale .* B_O_R * x, [B_A2_R * ê[1], B_A2_R * ê[2], B_A2_R * ê[3]]))
ns32 = GLMakie.Observable(map(x -> smallarrowscale .* B_O_R * x, [B_A3_R * ê[1], B_A3_R * ê[2], B_A3_R * ê[3]]))
ns42 = GLMakie.Observable(map(x -> smallarrowscale .* B_O_R * x, [B_A4_R * ê[1], B_A4_R * ê[2], B_A4_R * ê[3]]))
GLMakie.arrows!(lscene2,
    pivot_ps2, pivot_ns2, fxaa = true, # turn on anti-aliasing
    color = [:red, :green, :blue],
    linewidth = linewidth1, arrowsize = arrowsize1,
    align = :center
)
GLMakie.arrows!(lscene2,
    ps12, ns12, fxaa = true, # turn on anti-aliasing
    color = [:red, :green, :blue],
    linewidth = linewidth1, arrowsize = arrowsize1,
    align = :center
)
GLMakie.arrows!(lscene2,
    ps22, ns22, fxaa = true, # turn on anti-aliasing
    color = [:red, :green, :blue],
    linewidth = linewidth1, arrowsize = arrowsize1,
    align = :center
)
GLMakie.arrows!(lscene2,
    ps32, ns32, fxaa = true, # turn on anti-aliasing
    color = [:red, :green, :blue],
    linewidth = linewidth1, arrowsize = arrowsize1,
    align = :center
)
GLMakie.arrows!(lscene2,
    ps42, ns42, fxaa = true, # turn on anti-aliasing
    color = [:red, :green, :blue],
    linewidth = linewidth1, arrowsize = arrowsize1,
    align = :center
)
...
GLMakie.update_cam!(lscene2.scene, GLMakie.Vec3f(eyeposition...), GLMakie.Vec3f(lookat...), GLMakie.Vec3f(up...))
...
errormonitor(@async while (isopen(clientside) && run)
    ...
    flag = "R1" in keys(readings) && "R2" in keys(readings) &&
           ...
           "beta" in keys(readings) && "gamma" in keys(readings) &&
    ...
    if flag
        ...
        R3 = readings["R3"]
        R4 = readings["R4"]
        fused_beta = readings["beta"]
        fused_gamma = readings["gamma"]
        v1 = readings["v1"]
        v2 = readings["v2"]
        ...
        γ = atan(ĝ[2], ĝ[3])
        println("β: $β, fused β: $fused_beta, γ: $γ, fused γ: $fused_gamma.")
        global pitch = 0.5 * fused_beta / 180.0 * π + 0.5 * pitch
        global roll = 0.5 * fused_gamma / 180.0 * π + 0.5 * roll
        pitch2 = atan(-R2[1], √(R2[2]^2 + R2[3]^2))
        ...
        GLMakie.rotate!(robot2, GLMakie.Quaternion(g2))
        ...
        pivot_observable2[] = GLMakie.Point3f(O_B_R2 * (pivot - origin) + origin)
        point1_observable2[] = GLMakie.Point3f(O_B_R2 * (p1 - origin) + origin)
        point2_observable2[] = GLMakie.Point3f(O_B_R2 * (p2 - origin) + origin)
        point3_observable2[] = GLMakie.Point3f(O_B_R2 * (p3 - origin) + origin)
        point4_observable2[] = GLMakie.Point3f(O_B_R2 * (p4 - origin) + origin)
        ...
        _ps[] = [GLMakie.Point3f(point1_observable2[]...), GLMakie.Point3f(point2_observable2[]...), GLMakie.Point3f(point3_observable2[]...), GLMakie.Point3f(point4_observable2[]...)]
        _ns[] = map(x -> x .* arrowscale, [GLMakie.Vec3f(O_B_R2 * _R1...), GLMakie.Vec3f(O_B_R2 * _R2...),
                                           GLMakie.Vec3f(O_B_R2 * _R3...), GLMakie.Vec3f(O_B_R2 * _R4...)])
        ...
        pivot_ps2[] = [GLMakie.Point3f(pivot_observable2[]...), GLMakie.Point3f(pivot_observable2[]...), GLMakie.Point3f(pivot_observable2[]...)]
        ps12[] = [GLMakie.Point3f(point1_observable2[]...), GLMakie.Point3f(point1_observable2[]...), GLMakie.Point3f(point1_observable2[]...)]
        ps22[] = [GLMakie.Point3f(point2_observable2[]...), GLMakie.Point3f(point2_observable2[]...), GLMakie.Point3f(point2_observable2[]...)]
        ps32[] = [GLMakie.Point3f(point3_observable2[]...), GLMakie.Point3f(point3_observable2[]...), GLMakie.Point3f(point3_observable2[]...)]
        ps42[] = [GLMakie.Point3f(point4_observable2[]...), GLMakie.Point3f(point4_observable2[]...), GLMakie.Point3f(point4_observable2[]...)]

        ns12[] = map(x -> x .* LinearAlgebra.norm(R1) .* smallarrowscale, [B_O_R2 * B_A1_R * ê[1], B_O_R2 * B_A1_R * ê[2], B_O_R2 * B_A1_R * ê[3]])
        ns22[] = map(x -> x .* LinearAlgebra.norm(R2) .* smallarrowscale, [B_O_R2 * B_A2_R * ê[1], B_O_R2 * B_A2_R * ê[2], B_O_R2 * B_A2_R * ê[3]])
        ns32[] = map(x -> x .* LinearAlgebra.norm(R3) .* smallarrowscale, [B_O_R2 * B_A3_R * ê[1], B_O_R2 * B_A3_R * ê[2], B_O_R2 * B_A3_R * ê[3]])
        ns42[] = map(x -> x .* LinearAlgebra.norm(R4) .* smallarrowscale, [B_O_R2 * B_A4_R * ê[1], B_O_R2 * B_A4_R * ê[2], B_O_R2 * B_A4_R * ê[3]])
        ...
        GLMakie.rotate!(rollingwheel2, mq)
        GLMakie.rotate!(reactionwheel2, rq)
    end
end)
