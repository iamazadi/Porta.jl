...
tori = []
for i in 1:number
    push!(tori, Torus(Biquaternion(tail), scene, r = basepointradius, R = 0.1,
...
function animate(i::Int)
    ...
    for index in 1:number
        ...
        rgb = quaternionicrgb(Quaternion(p), Quaternion(1, 0, 0, 0), 0.8 + 0.2 * rand(), 0.8 + 0.2 * rand())
        rgb = 0.8 .* rgb + 0.2 .* rand(3)
        solidcolor = Makie.RGBAf(rgb..., 0.5 + 0.25 * rand())
        ghostcolor = Makie.RGBAf(rgb..., 0.2 + 0.1 * rand())
