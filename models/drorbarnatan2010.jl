...
function build(scene, surface, color)
    ...
    Makie.surface!(scene, x, y, z, color = color)
    x, y, z
end
...
function pullback(p::Geographic, α::Real, angle::Real=0)
    ...
    ϕ, θ = r .* (cos(ϕ), sin(ϕ))
    ϕ, θ = ϕ + pi, (θ + pi / 2) / 2
    Quaternion(ComplexPlane(sin(θ) * exp(im * (ϕ + α) / 2), cos(θ) * exp(im * (α - ϕ) / 2)))
end
...
"""
function getsurface(τ::Real, q::Quaternion, p::Array{Geographic,1}, s::Int)
    number = length(p)
    surface = Array{ℝ³}(undef, s, number)
    lspace = range(α₁, stop = α₂, length = s)
    for (i, α) in enumerate(lspace)
        for j in 1:number
            surface[i, j] = σmap(rotate(q, pullback(p[j], α, τ)))
        end
    end
    surface
end
...
function animate(i)
    ...
    for (p, nodes) in zip(points, observables)
        x, y, z = nodes
        surface = getsurface(τ, Quaternion(0, ℝ³(0, 0, 1)), p, segments)
        x[] = map(i -> vec(i)[1] , surface[:, :])
        y[] = map(i -> vec(i)[2] , surface[:, :])
        z[] = map(i -> vec(i)[3] , surface[:, :])
    end
    x, y, z = s2observables
    s2 = sphere(Quaternion(τ/2, ℝ³(0, 0, 1)), τ + α₁, segments)
    x[] = map(i -> vec(i)[1] , s2[:, :])
    y[] = map(i -> vec(i)[2] , s2[:, :])
    z[] = map(i -> vec(i)[3] , s2[:, :])
...
eyeposition = 0.8 .* Makie.Vec3f0(1, 3, 0)
lookat = Makie.Vec3f0(0.5, 0, 0)
