...
center = [0.0; 0.0; 0.0]
for i in eachindex(boundary_nodes[1])
    geographic = convert_to_geographic(boundary_nodes[1][i])
    global center = center + geographic
end
center[1] = 1.0
center[2] = center[2] ./ length(boundary_nodes[1])
center[3] = center[3] ./ length(boundary_nodes[1])
center = convert_to_cartesian(center)
x, y, z = center
λ₀ = x / (1 - z) + im * y / (1 - z)
index = Int(floor(length(boundary_nodes[1]) / 2))
x, y, z = boundary_nodes[1][index]
firstnode = x / (1 - z) + im * y / (1 - z)
vector = firstnode - λ₀
r₀ = 5norm(vector)
