...
modelname = "segment25_gamma3_$version"
L = 10.0 # max x range
L′ = -L
x̂ = [1.0; 0.0; 0.0]
ŷ = [0.0; 1.0; 0.0]
...
for i in eachindex(boundary_nodes)
    ...
    w = [τ(boundary_nodes[i][j], ϕ) for j in eachindex(boundary_nodes[i])]
...
basemap1 = Basemap(lscene, x -> G(θ1, τ(x, ϕ)), basemapsegments, basemap_color, transparency = true)
basemap2 = Basemap(lscene, x -> G(θ2, τ(x, ϕ)), basemapsegments, basemap_color, transparency = true)
...
for i in 1:steps_number - 1
    ...
    bandcolor = GLMakie.Observable(GLMakie.HSVA(Int(floor(float(i) / float(steps_number - 1) * 360.0)), 100, 100, 2α))
...
function animate(stage, progress)
    ...
    transform() = begin
        ...
        update!(basemap1, x -> G(θ1, τ(x, ϕ)) * q)
        update!(basemap2, x -> G(θ2, τ(x, ϕ)) * q)
        for (i, whirl) in enumerate(whirls)
            _w = τ.(boundary_nodes[i], ϕ)
            _w = map(x -> x * q, _w)
